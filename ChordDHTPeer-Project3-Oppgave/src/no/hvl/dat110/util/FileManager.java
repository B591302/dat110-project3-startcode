...
public class FileManager {
	...
	public void createReplicaFiles() {
		...
		// hash the replica
			BigInteger storeFile = Hash.hashOf(Rep);
		// store the hash in the replicafiles array.
			replicafiles[i] = storeFile;
		}
    ...
    public int distributeReplicastoPeers() throws RemoteException {
    	...
    	for(int i = 0; i < replicafiles.length; i++) {
    	 ...
    	 NodeInterface sucessor = chordnode.findSuccessor(replica);
    	
    	
    	// call the addKey on the successor and add the replica
    	 sucessor.addKey(replica);
    	
    	// call the saveFileContent() on the successor
    	sucessor.saveFileContent(filename, replica, bytesOfFile, counter==index);
    	// increment counter
    	counter++;
	...
	public NodeInterface findPrimaryOfItem() {
		...
		// use the primaryServer boolean variable contained in the Message class to check if it is the primary or not
			NodeInterface primary = Util.getProcessStub(m.getNodeIP(), m.getPort());
		// return the primary
			return primary;
		}
